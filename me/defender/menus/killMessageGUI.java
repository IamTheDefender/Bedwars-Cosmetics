...
public class killMessageGUI extends HInventory {

    String title = "Kill Messages";
    String loc = "KillMessages";
    String loc1 = "KillMessage";
    String perm = "KillMessages";
    FileConfiguration config = util.plugin().killmessagecfg.getConfig();

    public killMessageGUI() {
        super("none", "Kill Messages", 6, InventoryType.CHEST);
    }
    ...
    protected void onOpen(@NotNull Player player) {
        ...
        for(String name : config.getConfigurationSection(loc).getKeys(false)) {
            ...
            lore = util.formatLore(lore, name, util.getItemStatus(player, loc1, perm + "." + name, name, price), price);
            List<String> lore1 = new ArrayList<>();
            for(String lores : lore){
                lore1.add(lores);
            }
            if(material != null) {
                item = new ClickableItem(HCore.itemBuilder(Material.valueOf(material)).name(true, "&a" + name).durability(data).lores(true, lore1).build(), (e) -> {
                    util.onClick(player, e.getCurrentItem(), loc1, this, perm, price);
                });
            }else{
