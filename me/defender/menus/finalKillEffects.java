...
public class finalKillEffects extends HInventory {
    String title = "Final Kill Effects";
    String loc = "FinalKillEffects";
    String perm = "finalkilleffect";
    FileConfiguration config = util.plugin().finalkilldata.getConfig();

    public finalKillEffects() {
        super("none", "Final Kill Effects", 6, InventoryType.CHEST);
    }
    ...
    protected void onOpen(@NotNull Player player) {
        ...
        for(String name : config.getConfigurationSection(loc).getKeys(false)) {
            ...
            super.setItem(49, HCore.itemBuilder(Material.ARROW).name(true, "&aBack").build(), (e) -> {
